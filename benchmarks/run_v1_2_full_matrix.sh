#!/usr/bin/env bash
# Full 7-model x 3-context real kakeyaturbo v1.2 (A+B') benchmark.
set -euo pipefail

BASE="${BASE:-/workspace}"
OUT_ROOT="${OUT_ROOT:-$BASE/reports/real_kakeyaturbo_v1_2/full}"
LOG="${LOG:-/tmp/v1_2_full.log}"

mkdir -p "$OUT_ROOT"
echo "=== kakeyaturbo v1.2 FULL matrix start $(date -u +%FT%TZ) ===" | tee "$LOG"
...
for row in "${ROWS[@]}"; do
  ...
  for ctx in "${CONTEXTS[@]}"; do
    ...
    python3 "$BASE/benchmarks/kakeyaturbo_v1_2_real_bench.py" \
      ...
      --prefill-chunk "$chunk" \
      --out-dir "$outdir" 2>&1 | tee -a "$LOG"
  done
done

echo "" | tee -a "$LOG"
echo "=== kakeyaturbo v1.2 FULL matrix end $(date -u +%FT%TZ) ===" | tee -a "$LOG"
echo V1_2_DONE > /tmp/v1_2_done.flag
