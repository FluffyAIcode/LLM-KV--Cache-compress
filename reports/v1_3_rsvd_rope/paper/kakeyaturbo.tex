...
\begin{abstract}
We introduce \textsc{KakeyaTurbo}, a post-hoc compression codec for the key--value (KV) cache of transformer-based large language models. The codec constructs a \emph{Kakeya-like skeleton} of each per-block KV manifold via a weighted randomized singular value decomposition with an explicit rank cap $r = D/2$, applies a per-architecture inverse rotary preprocessor to the key stream when applicable, and passes the residual through a TurboQuant-style Walsh--Hadamard-rotated Lloyd--Max scalar quantizer at $2$ bits per coordinate. The three stages are unified under a single rate--distortion objective parameterized by a compile-time distortion metric $\rho$ and a per-vector weight $w_i$.

\textsc{KakeyaTurbo} operates in the full inference pipeline without requiring end-to-end offline capture. A block-streaming mode --- 512 tokens of uncompressed ``hot tail'' + asynchronous block-ready encode trigger --- makes the codec compatible with prefill (batched encode over $\lceil N / 512 \rceil$ blocks), token-by-token decode (decode-on-demand of queried blocks at $O(D \cdot d_{\text{eff}})$ per vector, $<10\,\mu$s per layer on H100), continuous batching, and paged-attention runtimes (vLLM / SGLang / TensorRT-LLM); a strict $O(1)$-per-vector streaming variant is available by substituting Frequent Directions for randomized SVD while preserving the same rate--distortion guarantees.

While preserving the block-streaming compression ratio, we quantify the quality cost along three axes: K-stream mean-squared reconstruction error under the final $b=2$ + randomized-SVD configuration inflates by $1.08\times$--$1.16\times$ versus the exact-PCA $b=3$ baseline across six of seven benchmark models (ACCEPT to MARGINAL on the project's standard $[1.0, 1.1, 1.3]\times$ verdict scale) and actually \emph{improves} by $0.42\times$ on Gemma-4 as a genuine rate--distortion-optimal side-effect; V-stream MSE inflates $1.07\times$--$1.22\times$ on the same six models; head-to-head on identical captured tensors, \textsc{KakeyaTurbo}'s K-stream MSE is $62\times$ to $2428\times$ lower than TurboQuant turbo3 on the Qwen / DeepSeek family.

We ground these gains in Shannon's rate--distortion framework and prove that the randomized-SVD skeleton sits at the shallow end of an explicit four-step literature-traceable dependency chain --- Guth's multilinear Kakeya endpoint estimate $\to$ Bennett--Carbery--Christ--Tao Brascamp--Lieb equivalence $\to$ Tropp's matrix Chernoff inequality $\to$ Halko--Martinsson--Tropp RSVD error bound --- whose deep end is used by Wang and Zahl in their recent resolution of the three-dimensional Euclidean Kakeya conjecture. We state a matching rate--distortion sandwich: an unconditional upper bound (Proposition~\ref{prop:rsvd-skeleton}) from the RSVD analysis, and a conjectural lower bound (Proposition~\ref{prop:kakeya-rd}) from the Kakeya-maximal-function estimates; in $D = 3$ the Wang--Zahl theorem closes the sandwich unconditionally. The $r = D/2$ rank cap is thereby shown to be a principled Pareto-optimal choice on the rate--distortion curve, not an efficiency heuristic.

On real bf16 KV tensors from seven open-source models (Qwen2.5, Qwen3, Gemma-4, DeepSeek-R1-Distill, GLM-Edge $\times 2$, SmolLM2) at $4\,096$ context tokens, \textsc{KakeyaTurbo} outperforms every post-hoc compression algorithm we compare against: tier-1 ($b=2$, $r=D/2$) beats TurboQuant turbo3 by $+9.1\%$ to $+29.2\%$ on 6 of 7 models at ACCEPT-level quality; tier-1.5 (with inverse-RoPE) reaches $+30\%$ to $+39\%$ on the Qwen / DeepSeek family. Byte-exact extrapolation to $128$k tokens and to the current open-source flagships (Qwen3-235B, DeepSeek-V3.1, Kimi-K2, GLM-4.6, MiniMax-M2) is reported. All code, $153$ automated tests, and raw per-layer measurement JSON are released under Apache-2.0.
\end{abstract}
...
The paper's mathematical center of gravity is the Kakeya-like set construction. We show in \S\ref{sec:kbl-chain} that the randomized-SVD skeleton of \S\ref{sec:skeleton} sits at the shallow end of an explicit four-step technical chain --- Guth's multilinear Kakeya endpoint~\cite{guth-endpoint} $\to$ Brascamp--Lieb inequality~\cite{bennett-carbery-christ-tao} $\to$ Tropp's matrix Chernoff inequality~\cite{tropp-matrix-chernoff} $\to$ the Halko--Martinsson--Tropp RSVD error bound~\cite{halko-martinsson-tropp} --- whose deep end is used by Wang and Zahl~\cite{wang-zahl} in their recent resolution of the three-dimensional Euclidean Kakeya conjecture. This is not an informal analogy but a literature-traceable dependency graph: Wang--Zahl depends on Guth's multilinear Kakeya; matrix concentration depends on Brascamp--Lieb via the Carbery--Valdimarsson duality~\cite{carbery-valdimarsson}; RSVD depends on matrix concentration. The remainder of the paper is empirical.
