...
The codec is designed for block-streaming operation in an LLM inference pipeline rather than offline batch capture. A 512-token uncompressed hot tail plus an asynchronous block-ready encode trigger makes the codec compatible in principle with prefill (batched encode over $\lceil N / 512 \rceil$ blocks), token-by-token decode (queried-block decoding at $O(D \cdot d_{\text{eff}})$ per vector; a FLOPs estimate gives $\lesssim 10\,\mu$s per layer on H100, which is \emph{not} directly measured in this paper's CPU-only benchmark environment), continuous batching, and paged-attention runtimes; a strict $O(1)$-per-vector streaming variant follows from substituting Frequent Directions for randomized SVD. Production integrations (vLLM / SGLang / TensorRT-LLM) are out of scope for this paper's measurements and are documented in the associated repository.

We give a partial rate--distortion characterisation of \textsc{KakeyaTurbo} in Shannon's framework. The analysis uses an explicit literature-traceable chain --- Guth's multilinear Kakeya endpoint $\to$ Bennett--Carbery--Christ--Tao Brascamp--Lieb equivalence $\to$ Tropp's matrix Chernoff inequality $\to$ Halko--Martinsson--Tropp RSVD error bound --- whose deep end is used by Wang and Zahl in their recent resolution of the three-dimensional Euclidean Kakeya conjecture. This yields an \emph{unconditional} upper bound on reconstruction error (Proposition~\ref{prop:rsvd-skeleton}) and a \emph{conditional} lower bound (Proposition~\ref{prop:kakeya-rd}) that depends on the Kakeya-maximal-function conjecture in dimension $D$; the two bounds meet unconditionally only at $D = 3$ via Wang--Zahl, while at the KV head dimensions of practical interest ($D \in \{64, 128, 192, 256, 512\}$) the lower bound remains conditional and the Pareto-optimality of $r = D/2$ is therefore \emph{argued} --- not proved --- via the partial closure at $D = 3$ and the upper-bound analysis. A quality audit using reconstruction MSE (a tractable upper bound on attention-logit perturbation for bounded-norm queries) on real bf16 KV tensors reports inflation of $1.08\times$--$1.16\times$ versus the exact-PCA $b=3$ baseline on six of seven benchmark models, and a $0.42\times$ improvement on Gemma-4 as a genuine Lloyd--Max RDO side-effect.
...
We define \textsc{KakeyaTurbo}, a post-hoc, training-free, three-stage codec that composes a per-block Kakeya-like skeleton (via weighted randomized SVD with a hard rank cap $r = D/2$), a per-architecture inverse rotary preprocessor on the key stream (where applicable), and a Walsh--Hadamard-rotated Lloyd--Max scalar quantizer on the residual at $2$ bits per coordinate. The three stages are not glued heuristics: they are the \emph{separable} optimisers of three nested subproblems of a single rate--distortion objective~\eqref{eq:rd}. Section~\ref{sec:framework} derives the objective; Section~\ref{sec:implementation} gives the algorithm, the design contract (no \texttt{unsafe}, no dynamic dispatch, compile-time monomorphisation), and a block-streaming operational mode designed for live prefill / decode / paged-attention pipelines. We emphasise that this paper does not report runtime measurements inside any specific serving stack: all timings are CPU measurements in an isolated-codec environment, and cross-stack integration is documented in the associated repository as future work.

\paragraph{Contribution 2: a partial RD characterisation of KakeyaTurbo.}
We place \textsc{KakeyaTurbo} in Shannon's rate--distortion framework and establish one unconditional and one conditional bound. The upper bound (Proposition~\ref{prop:rsvd-skeleton}) is unconditional and follows from the Halko--Martinsson--Tropp analysis of randomized SVD; it bounds reconstruction error by the $(d_{\text{eff}}+1)$-th singular value of the centred data. The lower bound (Proposition~\ref{prop:kakeya-rd}) is conditional on the Kakeya-maximal-function conjecture in dimension $D$; at $D = 3$ the recent theorem of Wang and Zahl~\cite{wang-zahl} closes the lower bound unconditionally. The two bounds are linked by an explicit four-step literature-traceable dependency chain --- Guth's multilinear Kakeya endpoint~\cite{guth-endpoint} $\to$ Bennett--Carbery--Christ--Tao Brascamp--Lieb equivalence~\cite{bennett-carbery-christ-tao} $\to$ Tropp's matrix Chernoff inequality~\cite{tropp-matrix-chernoff} $\to$ the Halko--Martinsson--Tropp RSVD error bound~\cite{halko-martinsson-tropp}. Section~\ref{sec:framework} and \S\ref{sec:kbl-chain} construct the chain.

We are explicit about what this does and does not prove. At $D = 3$ the pair of bounds meet and the rank cap $r = D/2$ is provably Pareto-optimal; at the head dimensions of practical interest ($D \in \{64, 128, 192, 256, 512\}$) the lower bound remains conjectural, so we obtain \emph{evidence for} rather than \emph{proof of} Pareto-optimality at deployment scale. Bridging this gap either requires establishing the Kakeya-maximal-function conjecture in higher dimensions (an open problem in harmonic analysis) or replacing it with a weaker hypothesis specific to the KV distribution, which is beyond this paper.
...
We will instantiate $f$ as a three-stage composition (rotation, quantised-skeleton projection, residual codebook) in \S\ref{sec:parameterisation} and give a partial characterisation of its RD behaviour in \S\ref{sec:kbl-chain} --- unconditional on the upper side, conditional (outside $D=3$) on the lower side. The rest of this section fixes the geometric object that enters the characterisation --- a Kakeya-like set of directions spanned by $X$ --- and traces its literature.
...
Let $\Theta = \{x_i / \norm{x_i}\}_{i=1}^n \subset S^{D-1}$, and let $N(\Theta, \delta)$ denote the $\delta$-covering number of $\Theta$ in the chordal metric. Suppose $\Theta$ is $\delta$-separated for some $\delta > 0$ and let $d_\delta(\Theta) := \log N(\Theta, \delta) / \log(1/\delta)$ denote its \emph{metric entropy dimension at scale $\delta$}. If the Kakeya-maximal-function conjecture holds in dimension $D$, then the minimum byte budget to reconstruct all $x_i$ to $\ell_2$-error $\varepsilon\norm{x_i}$ satisfies
\begin{equation}
B(\varepsilon, n, D) \;=\; \Omega\!\left(n \cdot \log_2\!\frac{1}{\varepsilon} \,+\, d_\delta(\Theta) \cdot \log_2\frac{1}{\varepsilon}\right)
\end{equation}
for any $\delta \leq \varepsilon$. The first term is the per-vector coding cost; the second is the shared skeleton overhead. This is the conjectural complement to the \emph{unconditional} upper bound of Proposition~\ref{prop:rsvd-skeleton}; at $D = 3$ and for $\delta$-nets, the matching lower bound is provided unconditionally by the Wang--Zahl theorem~\cite{wang-zahl}. For $D \geq 4$ the lower bound remains conditional on the Kakeya-maximal-function conjecture (open).

\emph{Note on dimension.} We use the metric entropy dimension $d_\delta$ rather than the Hausdorff dimension $\dim_H$, because $\Theta$ is a finite set and $\dim_H(\Theta) = 0$ trivially. The quantity $d_\delta(\Theta)$ is well-defined for any finite $\delta$-separated set, reduces to the classical Minkowski/box-counting dimension as $\delta \to 0$ in the continuum limit, and is the right object for the scale-$\delta$ tube-packing arguments that underlie the Kakeya lower-bound program.
\end{proposition}
...
\begin{tabular}{lll}
  ...
  \midrule
  Distortion $\rho$ & MSE (measured); upper bound on attention perturbation & see note below on the MSE-vs-InnerProduct choice \\
  ...
  Rank cap $r$ & $r = D/2$ & Pareto-optimal at $D = 3$ (Prop~\ref{prop:rsvd-skeleton},~\ref{prop:kakeya-rd}); conjectural at higher $D$ \\
  Codebook & spherical $k$-means $+$ Lloyd--Max Gaussian & $k$-means on skeleton coefficients; Lloyd--Max on rotated residual \\
  \bottomrule
\end{tabular}
\end{table}

\paragraph{MSE as the distortion throughout.}
Earlier drafts of this work proposed an asymmetric distortion ($\rho = $ MSE on V, $\rho = $ InnerProduct on K) on the grounds that attention consumes K through dot products and V through linear combinations. For this paper's experiments we report MSE throughout, for both streams, for two reasons: (i) MSE is a uniform upper bound on InnerProduct distortion, since for any unit-norm query $q$,
\[
|q^\top (k - \hat k)| \leq \norm{q} \cdot \norm{k - \hat k} = \norm{k - \hat k},
\]
so an MSE ACCEPT verdict implies an InnerProduct ACCEPT verdict for any bounded-norm query; (ii) an InnerProduct distortion is query-dependent, and a query-conditional codec is out of scope here. The resulting MSE measurements are therefore \emph{conservative} proxies for the attention-logit-perturbation quantity that actually matters for downstream quality; the true attention-logit loss is at most the reported MSE loss.

Two further observations: the five parameters are \emph{separable} optimisers of three nested subproblems of~\eqref{eq:rd} (rotation $\to$ skeleton $\to$ residual), with each subproblem admitting a closed-form optimum up to RSVD randomisation; and the rank cap $r = D/2$ is the non-trivial choice, discussed in detail in \S\ref{sec:kbl-chain}.
...
\begin{enumerate}[label=(\roman*), leftmargin=*]
  ...
  \item (\emph{$\varepsilon$-rank tightness}) Define the $\varepsilon$-numerical rank of the centred design matrix $A := [x_1 - \mu; \ldots; x_n - \mu]^\top$ as $r_\varepsilon(A) := \min\{k : \sigma_{k+1}(A) / \sigma_1(A) \leq \varepsilon\}$. Then $d_{\text{eff}} \leq r_\varepsilon(A) + O(p)$ for any $\varepsilon$ matching the variance-ratio threshold of Algorithm~\ref{alg:rsvd}. When the rows of $A$ additionally concentrate on a $\delta$-net $\Theta$ in angle, $r_\varepsilon(A) \leq d_\delta(\Theta) + O(\log(1/\varepsilon))$, where $d_\delta(\Theta)$ is the metric entropy dimension of \S\ref{sec:framework}.
\end{enumerate}
\end{proposition}

\begin{proof}[Proof sketch]
(i) is the Halko--Martinsson--Tropp bound~\eqref{eq:hmt-bound} restated as angular coverage: the coefficient $c_i = U^\top(x_i - \mu)$ reconstructs $x_i$ with error $\norm{x_i - \mu - U c_i}$, and the squared sum of these errors is bounded by the trailing singular values, which by the HMT bound are at most $(1 + \epsilon_{\text{RSVD}})^2 \sigma_{d_{\text{eff}}+1}^2$ up to a rank factor.

(ii) has two parts. The first inequality $d_{\text{eff}} \leq r_\varepsilon(A) + O(p)$ follows directly from the variance-ratio truncation rule of Algorithm~\ref{alg:rsvd}: once the cumulative squared singular values exceed the ratio threshold, truncation stops. The second inequality $r_\varepsilon(A) \leq d_\delta(\Theta) + O(\log 1/\varepsilon)$ is a standard metric-entropy estimate: an $\varepsilon$-accurate reconstruction requires a basis whose rank exceeds the $\delta$-covering number of the angular distribution of rows in $A$, a reduction that does not require the Kakeya-maximal-function conjecture.
\end{proof}

Proposition~\ref{prop:rsvd-skeleton} is the upper-bound formalisation of the ``design dictum'' from \S\ref{sec:framework}. The skeleton $\mathcal{K}_U$ is a Kakeya-like set relative to the \emph{data direction set} $\Theta$ (as distinct from the classical $S^{D-1}$), with coverage radius bounded by HMT and rank matching the metric entropy dimension of $\Theta$ up to logarithmic factors. The lower-bound complement (Proposition~\ref{prop:kakeya-rd}) is where the Kakeya-maximal-function conjecture enters, and we stress that Prop.~\ref{prop:rsvd-skeleton} itself is unconditional.
...
Wang--Zahl's proof constructs a Frostman measure on $\mathcal{K}$ whose Fourier decay rate encodes $\dim_H(\mathcal{K})$ (defined on the continuous Kakeya set). Our discrete counterpart is the cumulative singular-value distribution $F(k) = \sum_{i \leq k} \sigma_i^2 / \sum_i \sigma_i^2$; the variance-ratio threshold used to pick $d_{\text{eff}}$ plays the role of a discrete Frostman-dimension cutoff, matching $r_\varepsilon(A)$ above. The parallel is structural, not a literal transport of $\dim_H$ to a finite sample.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Classical direction set.} The hardest instance of the Kakeya conjecture is $\Theta = S^{D-1}$ (full continuous sphere, where $\dim_H$ is well-defined and equals $D - 1$). In our setting $\Theta$ is a finite set of at most $n$ directions, and its metric entropy dimension $d_\delta(\Theta)$ at the relevant scales is empirically much smaller than $D - 1$ (otherwise KV cache would not be compressible at all). The codec operates in this ``easy'' discrete regime; Wang--Zahl operates in the ``hardest'' continuous regime where the covering assumption saturates.
...
For each codec configuration and each layer we compute the mean per-block reconstruction MSE on both K and V streams, excluding layer 0 (RoPE-degenerate). MSE is used uniformly across K and V, despite the different role the two streams play in attention, for the reason explained in \S\ref{sec:parameterisation}: MSE is a bounded-norm-query upper bound on the attention-logit perturbation that K actually feeds into, so an MSE ACCEPT verdict is a conservative proxy for an InnerProduct ACCEPT verdict. We report MSE \emph{inflation} against the v1.2 baseline (exact PCA, $b=3$) and adopt the same acceptance thresholds used in every ablation of the project:
\begin{center}
\begin{tabular}{ll}
  \toprule
