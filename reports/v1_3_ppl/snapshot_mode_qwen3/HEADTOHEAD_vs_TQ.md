...
| Metric                            | bf16 baseline | TurboQuant k8v4 | `v1.3-GPU-snapA` |
|:----------------------------------|:-------------:|:---------------:|:---------------:|
| PPL (mean over passages)          |       11.836  |         11.847  |         22.982  |
| PPL (median over passages)        |        7.933  |          7.909  |         10.954  |
| Δppl vs bf16 (mean)               |           —   |       **+0.09%** |       +94.17%   |
| Δppl vs bf16 (paired snapshot)    |           —   |       **+0.09%** |       +61.84%   |
| top-1 agreement                   |        100%   |        ~100%    |        79.30%   |
| K reconstruction MSE (abs)        |           0   |      **0.0048** |        0.503    |
| K relative MSE (MSE / var(K))     |           0   |     **0.0002**  |        ~0.02    |
| V reconstruction MSE (abs)        |           0   |       **0.0525** |        0.0517   |
| V relative MSE (MSE / var(V))     |           0   |       **0.0206** |        ~0.02    |
| Compression ratio (per token)     |        1.00×  |       **2.61×** |         2.20×   |
| Compression (blended w/ 14-layer bf16 skip) |  1.00×  |       **2.61×** |         1.50×   |
| Forward time per passage (pass-2) |       0.134 s |       0.148 s  |       **0.064 s** |
| Pass-1 (clean forward)            |           —   |          —     |       0.342 s   |
| Offline codec time                |           —   |          —     |      **18.08 s** |
| Peak GPU memory (all 4 passages)  |      56.65 GiB |      56.63 GiB |      ~56.7 GiB  |
...
**2. `v1.3-GPU-snapA` loses 62-94 pp Δppl** and has K
...
every axis we measured, including top-1 agreement** (snapA's
79.30 % is 20 pp behind TQ's ~100 %).  For this model, TQ k8v4
is the correct production choice.
